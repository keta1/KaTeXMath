...
class MTTypesetter(
  ...
  companion object {
    fun createLineForMathList(
      ...
      val finalizedMathList = mathList.finalized()
      val preprocessedAtoms = preprocessMathList(finalizedMathList)
      val typesetter = MTTypesetter(font, style, cramped, spaced)
      typesetter.createDisplayAtoms(preprocessedAtoms)
      val lastAtom = finalizedMathList.atoms.lastOrNull()
      val maxrange = lastAtom?.indexRange?.maxrange ?: 0
      return MTMathListDisplay(typesetter.displayAtoms, NSRange(0, maxrange))
    }
